...
x<-qplot(x=factor(date_time$year+1900))+stat_bin(geom="text", aes(label=..count.., vjust=-1))+xlab("Year")+ylab("# of transactions")+ ggtitle("Yearly Demand graph")+
...
plot.title = element_text(face="bold",vjust=1.5,colour="black"))+coord_cartesian(ylim = c(0,4500))

ggsave("3_Demand by year.jpg", x)
